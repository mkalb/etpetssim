...
| Simulation            | Keep / current stance                                                                     | Practical additions                                                                                                                            | Not now / rationale                                                                                                                                                      | Erledigt                                                                                     |
|-----------------------|-------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------|
...
| Rebounding Entities   | Keep `Add wall`.                                                                          | Add `Remove wall`, parameterized `Add rebounder` with direction, and `Remove rebounder`.                                                       | Do not add global wall or moving-entity reinitialization; vertical walls and initial moving entity percentage are configuration concerns.                                | ✅ `Remove wall`, ✅ `Remove rebounder` (⏳ `Add rebounder`)                                    |
| Sugarscape            | No current edit actions.                                                                  | Add `Add agent`, `Remove agent`, parameterized `Set sugar` with practical levels (`empty`, `low`, `medium`, `high`), and `Remove sugar`.       | Avoid free numeric sugar editing for now; staged values are easier to expose in the toolbar and keep the UI compact.                                                     | ✅ `Add sugar` (parameterized), ✅ `Remove sugar` (⏳ `Add agent`, `Remove agent`, `Set sugar`) |
